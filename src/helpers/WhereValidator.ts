...
type ValidatorFunc = (cmp: any, datasets: Dataset[], datasetName: string, outerWhere?: boolean) => ValidationResult;
...
function validateIS(is: any, datasets: Dataset[], datasetName: string): ValidationResult {
	...
	if (!validateKeys(datasets, datasetName, field, "skey")) {
		return {valid: false, error: "IS has a key that is not a valid key"};
	}
...
function validateAnd(and: any, datasets: Dataset[], datasetName: string): ValidationResult {
	let loadedDatasets = datasets.map((each) => each.datasetName);
	if (and == null) {
	...
	for (let filter of and) {
		let res = validateWhere(filter, datasets, datasetName);
		if (!res.valid) {
			return res;
		}
	}
...
export function validateWhere(where: object, datasets: Dataset[],
	...
	if (validator) {
		...
		const result = validator((where as any)[keys[0]], datasets, datasetName);
		if (!result.valid) {
			return result;
		}
	}
...
function validateNot(not: any, datasets: Dataset[], datasetName: string): ValidationResult {
	let loadedDatasets = datasets.map((each) => each.datasetName);
	if (not === null) {
	...
	return validateWhere(not, datasets, datasetName);
}

function validateOr(or: any, datasets: Dataset[], datasetName: string): ValidationResult {
	let loadedDatasets = datasets.map((each) => each.datasetName);
	if (or == null) {
...
function checkCmp(cmp: any, datasets: Dataset[], datasetName: string): ValidationResult {
	let loadedDatasets = datasets.map((each) => each.datasetName);
	if (cmp == null) {
	...
	let field = key.split("_")[1];
	if (!validateKeys(datasets, datasetName, field, "mkey")) {
		return {valid: false, error: "GT, LT, or EQ has a key that is not a valid key"};
	}
